(function(){
  ...
  function filterPins(){var query=normalize(pinSearch?pinSearch.value:'').toLocaleLowerCase();var shown=0;pins.forEach(function(pin){var categoryMatch=activeFilter==='all'||pin.dataset.category===activeFilter;var text=normalize(pin.dataset.search+' '+pin.textContent).toLocaleLowerCase();var searchMatch=!query||text.indexOf(query)>-1;pin.hidden=!(categoryMatch&&searchMatch);if(!pin.hidden){shown+=1;}});if(pinStatus){pinStatus.textContent='显示 '+shown+' / '+pins.length+' 张卡片';}if(pinEmpty){pinEmpty.hidden=shown!==0;}}
  ...
  var queueCount=0;
  var pinErrorFields=pinForm?Array.prototype.slice.call(pinForm.elements).filter(function(field){return field.name;}):[];
  var categoryNames={tool:'工具',reading:'阅读',data:'数据',ops:'站务'};
  var categoryColors={tool:'pg44-pin-yellow',reading:'pg44-pin-sky',data:'pg44-pin-rose',ops:'pg44-pin-green'};
  var validReviews=['7','30','90'];
  var validStates=['待核验','已验证','本周使用'];
  ...
  function clearPreview(){currentCard=null;if(preview){preview.className='pg44-pin-preview';preview.innerHTML='<span>PIN / DRAFT</span><p class="pg44-preview-empty">完成左侧预检后，这里会生成一张可维护的收藏卡。</p>';}if(previewState){previewState.textContent='等待资料';}if(copyPin){copyPin.disabled=true;}if(queuePin){queuePin.disabled=true;}if(pinCopyStatus){pinCopyStatus.textContent='';}}
  function rejectPin(field,message){field.setAttribute('aria-invalid','true');pinMessage.textContent=message;field.focus();}
  function safeSource(value){
    if(/^www\./i.test(value)||(/^\w+:\/\//i.test(value)&&!/^https:\/\//i.test(value))){return false;}
    if(/^https:\/\//i.test(value)){try{var url=new URL(value);return !url.username&&!url.password&&Boolean(url.hostname);}catch(error){return false;}}
    return !/[<>\u0000-\u001f]/.test(value);
  }
  if(pinMessage){pinMessage.setAttribute('role','alert');pinMessage.setAttribute('aria-atomic','true');}
  if(pinForm){
    pinErrorFields.forEach(function(field){function invalidate(){field.removeAttribute('aria-invalid');pinMessage.textContent='';if(currentCard){clearPreview();previewState.textContent='资料已修改，请重新预检';}}field.addEventListener('input',invalidate);field.addEventListener('change',invalidate);});
    pinForm.addEventListener('submit',function(event){event.preventDefault();pinCopyStatus.textContent='';pinErrorFields.forEach(function(field){field.removeAttribute('aria-invalid');});var data=Object.fromEntries(new FormData(pinForm).entries());var title=normalize(data.title);var source=normalize(data.source);var reason=normalize(data.reason);
      if(!title){rejectPin(pinForm.elements.title,'请先填写收藏题名。');return;}
      if(countChars(title)>200){rejectPin(pinForm.elements.title,'收藏题名不能超过 200 个字符。');return;}
      if(!source){rejectPin(pinForm.elements.source,'请填写原始入口或来源名称。');return;}
      if(countChars(source)>500||!safeSource(source)){rejectPin(pinForm.elements.source,'请填写不含凭据的 HTTPS 安全链接，或改填纯文本来源名称。');return;}
      if(!Object.prototype.hasOwnProperty.call(categoryNames,data.category)){rejectPin(pinForm.elements.category,'请选择列表中的收藏类别。');return;}
      if(validReviews.indexOf(data.review)===-1){rejectPin(pinForm.elements.review,'请选择列表中的复核周期。');return;}
      if(validStates.indexOf(data.state)===-1){rejectPin(pinForm.elements.state,'请选择列表中的卡片状态。');return;}
      if(countChars(reason)<12||countChars(reason)>500){rejectPin(pinForm.elements.reason,'用途备注需为 12–500 个字符，并说明它会帮助哪个具体决定。');return;}
      pinMessage.textContent='';currentCard={title:title,source:source,category:data.category,review:data.review,state:data.state,reason:reason};
      preview.className='pg44-pin-preview '+categoryColors[data.category];preview.innerHTML='<span>PIN / READY</span><h3></h3><p class="pg44-preview-reason"></p><p class="pg44-preview-meta"></p>';
      preview.querySelector('h3').textContent=currentCard.title;preview.querySelector('.pg44-preview-reason').textContent=currentCard.reason;preview.querySelector('.pg44-preview-meta').textContent=categoryNames[currentCard.category]+' · '+currentCard.state+' · '+currentCard.review+' 天后复核';
      previewState.textContent='预检完成';copyPin.disabled=false;queuePin.disabled=false;
    });
    pinForm.addEventListener('reset',function(){setTimeout(function(){pinMessage.textContent='';pinErrorFields.forEach(function(field){field.removeAttribute('aria-invalid');});clearPreview();},0);});
    var sample=document.querySelector('[data-load-sample]');
    if(sample){sample.addEventListener('click',function(){pinForm.elements.title.value='中文长文排版检查清单';pinForm.elements.source.value='https://~SITE_DOMAIN~/article.html';pinForm.elements.category.value='tool';pinForm.elements.review.value='30';pinForm.elements.reason.value='用于发布前检查移动端断行、层级与键盘焦点。';pinForm.elements.state.value='本周使用';pinErrorFields.forEach(function(field){field.removeAttribute('aria-invalid');});clearPreview();pinMessage.textContent='示例已载入，可继续修改。';});}
  }
  ...
  if(lostForm){var lostInput=lostForm.querySelector('input');lostInput.addEventListener('input',function(){lostStatus.textContent='';lostResults.innerHTML='';lostResults.hidden=true;lostInput.removeAttribute('aria-invalid');});lostForm.addEventListener('submit',function(event){event.preventDefault();var query=normalize(lostInput.value).toLocaleLowerCase();lostResults.innerHTML='';if(!query){lostInput.setAttribute('aria-invalid','true');lostStatus.textContent='先输入一个主题关键词。';lostResults.hidden=true;lostInput.focus();return;}var found=boardIndex.filter(function(item){return normalize(item.title+' '+item.keys).toLocaleLowerCase().indexOf(query)>-1;});if(!found.length){lostStatus.textContent='当前板面没有匹配卡，试试“排版”或“数据”。';lostResults.hidden=true;return;}lostStatus.textContent='找到 '+found.length+' 张可访问卡片。';lostResults.hidden=false;found.forEach(function(item){var li=document.createElement('li');var a=document.createElement('a');a.href=item.href;a.textContent=item.title;li.appendChild(a);lostResults.appendChild(li);});});}
})();
