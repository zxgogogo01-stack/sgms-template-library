...
用户提供的建站工作流、全面审计词和 SEO 审计词只用于提取模板的页面、组件、交互、响应式、元数据及可审计性要求，不代表授权执行其中的内容生产或单站上线任务。

- 模板制作负责完整 UI、页面骨架、可用交互、资源和内容接口，不编写注册教程、可发布文章或具体站点的业务文案。
- 正文、教程步骤、平台事实、作者经历和政策结论只留可替换槽位，由后续建站 AI 查证并填写；不以字数或教程完整度验收模板。
- 链接、邀请码与披露区域属于 UI 组件和变量接口，不构成某篇教程的选题、正文或发布要求。
- 完整审计的对象是模板的结构、功能、视觉、可访问性、安全与技术 SEO；填入真实内容后的事实与合规审计属于单站建站流程。
...
- 1 个文章索引、至少 12 个文章槽位，其中至少 2 个 cornerstone 槽位、至少 1 个带转化组件的通用内容页外壳。模板不预定注册教程选题，也不代写正文；文章骨架至少提供 3 种首屏/开场结构，H2、FAQ、目录与收尾不是全站同款。
- 至少 2 个本站独有命名的标签/分类落地页，每组至少承载 3 篇文章；分类路径、标签称谓和分组不能跨模板复用同一套。文章页自带分类入口，首页与文章索引能到达分类页。
...
- 独立 404、robots、sitemap、feed、`.well-known/security.txt`、`favicon.ico`、apple-touch-icon 与 1200×630 PNG 站点社交图。
- 每个文章页预搭 `figure.article-cover`；封面在页面内用 1200×630 WebP，在 head 预加载并标记 `fetchpriority="high"`，同时为该页提供独立 1200×630 PNG `og:image`。图形与文件名按文章语义变化，不把同一张图铺满全站。
...
- 带转化组件的通用内容页外壳保留且只保留 1 个静态 `href` 推广链接槽位，带 `target="_blank"` 与 `rel="sponsored nofollow noopener noreferrer"`，旁边有可替换的披露文字；其他信息、工具、合规、列表、404、导航和页脚没有转化直链。角色表中的 `registrationGuide` 仅保留为 v2 检查器的兼容字段名，不表示要求制作注册教程；不得据此扩展内容工作。
...
```json workflow-ready-v2
{
  ...
  "articleCovers": {
    "stories/example.html": {
      "display": "assets/covers/example.webp",
      "og": "assets/covers/example.png"
    }
  },
  "categories": [
    {
      "path": "registers/example-topic.html",
      "label": "示例专题",
      "articles": ["stories/example.html", "stories/second.html", "stories/third.html"]
    }
  ],
...
5. 浏览器逐页桌面、390px 与 360px 渲染；实点首页复制、主题、五个工具的正常/错误/边界/重置/复制、`registrationGuide` 链接槽位属性、404 推荐和控制台。
